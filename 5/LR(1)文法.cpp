...
string text = "./Grammatical/Grammatical10.txt";
char Start;//开始符号
...
void Printf_LR1_GOTO() {
	cout << "LR(1)分析表:\n\n";
	printf("%-8s%-16s%-16s%-8s", "状态", "项目集", "后继符", "后继状态\n");
	Printf_(8);
	for (int i = 0; i < Analyze_Table.size(); i++) {
		printf("\n%s%-7d", "S", i);
		for (int j = 0; j < Analyze_Table[i].items.size(); j++) {
			string value = Analyze_Table[i].items[j].value;
			string Next_Char = SetToString_(Analyze_Table[i].items[j].successor);
			int Successor_Status = Analyze_Table[i].items[j].Successor_Status;
...
void Printf_ACTION_GOTO() {
	...
	cout << "|" << endl; Printf_((int)VT.size() + 1); cout << endl;
	for (int i = 0; i < Analyze_Table.size(); i++) {
		string In = "S" + Number_Turn_String(i);
		printf("%-6s", In.c_str());
		for (char c : VT) {
			printf("%-6s", ACTION_TABLE[i][c].c_str());
		}
		cout << "|" << endl; Printf_((int)VT.size() + 1); cout << endl;
	}
	cout << "\nGOTO表:\n";
	printf("%-6s\t%s\n", "状", "GOTO"); printf("%-6s", "态");
	for (char c : VN) if (c != Start)printf("%-6c", c);
	cout << "|" << endl; Printf_((int)VN.size()); cout << endl;
	for (int i = 0; i < Analyze_Table.size(); i++) {
		string In = "S" + Number_Turn_String(i);
		printf("%-6s", In.c_str());
		for (char c : VN) {
			if (c == Start)continue;
			if (GOTO_TABLE[i][c] == 0)printf("%-6s", "");
			else printf("%-6d", GOTO_TABLE[i][c]);
		}
		cout << "|" << endl; Printf_((int)VN.size()); cout << endl;
	}
}
